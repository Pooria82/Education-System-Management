...
public class ChangeStudentsGrades extends JFrame implements FindServices {
    ...
    public ChangeStudentsGrades(DataBase dataBase, String university, String faculty, String academic_department, Professor professor) {
                    ...
                    if (dataBase.getUniversity_list().get(i).getFaculty_list().get(j).getName().equals(faculty)) {
                        ...
                        for (int k = 0; k < dataBase.getUniversity_list().get(i).getFaculty_list().get(j).disciplines_list.size(); k++) {
                            if (dataBase.getUniversity_list().get(i).getFaculty_list().get(j).disciplines_list.get(k).getName().equals(academic_department)) {
                                ...
                                if (foundL && foundS) {
                                    JOptionPane.showMessageDialog(this, "The student's grade was successfully changed.!");
                                    dataBase.saveData();
                                    dispose();
                                }
                                break;
